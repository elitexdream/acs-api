...
class MaterialController extends Controller
{
    ...
    public function getTracks(Request $request) {
        ...
        $devices = $company->devices()->select('customer_assigned_name', 'id', 'serial_number')->whereIn('serial_number', $teltonika_ids)->get();

        $tracks = MaterialTrack::where('in_progress', false)
            ->whereIn('inventory_material_id', $inventory_materials->pluck('id'))
            ->select('id', 'inventory_material_id', 'start', 'stop')
            ->get();

        foreach ($tracks as $key => &$track) {
            $track->device_id = $track->inventoryMaterial->teltonika->teltonika_id;
        }

        return response()->json(compact('devices', 'tracks'));
    }
    ...
    public function getMaterialReport(Request $request) {
        ...
        $track = MaterialTrack::findOrFail($request->trackId);
